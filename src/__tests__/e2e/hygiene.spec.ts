...
const CRITICAL_PATHS = ["/", "/about/", "/ways-to-support/"] as const;
const VIEWPORTS = [
...
test.describe("runtime hygiene", () => {
  ...
  test("giving CTAs point to the right checkout campaigns", async ({ page, baseURL }) => {
    ...
    await expect(page.locator("#strong-circle .strong-circle-trigger")).toHaveAttribute(
      "href",
      "https://givebutter.com/22-strong-circle-bjf16z"
    );
    await expect(page.locator("#strong-circle .circle-tier").first()).toHaveAttribute(
      "href",
